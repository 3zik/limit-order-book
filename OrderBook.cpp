...
using Price    = std::int32_t;  // alias for readability
    ...
    Trades MatchOrders() {
        ...
        while (true) {
            ...
            while (bids.size() && asks.size()) {
                auto bid = bids.front(); // TIME-PRICE PRIORIITY. 
                                         // WE CHOOE the ONe that is there first (front of queue)
                auto ask = asks.front();
            ...
            // FIX (subtle but real): the empty-level cleanup used to live INSIDE the inner while.
            //      Erasing from bids_/asks_ invalidates the structured-binding references
            //      `bids`, `asks`, `bidPrice`, `askPrice`, and the next inner-loop iteration
            //      would then read `bids.size()` on a dangling reference → UB. Moving the
            //      cleanup out here is safe: the inner loop has already exited.
...
public:
    Trades AddOrder(OrderPointer order) {
        ...
        if (orders_.contains(order->GetOrderId())) // .contains() in a Cpp 20+ feature
        ...
        if (order->GetSide() == Side::Buy) {
            ...
            // FIX: was `std::next(order.begin(), order.size() - 1)` — `order` is a shared_ptr,
            //      it has no begin()/size(). Should be `orders` (the list we just pushed into).
            //      Also using std::prev(orders.end()) which is O(1) on a list vs O(n) walk.
    ...
    void CancelOrder(OrderId orderId) {
        ...
        // FIX (lifetime): the original erased from orders_ first and THEN used `order`/`iterator`.
        //      Those bindings refer into the now-deleted entry — dangling. Use them first, erase last.
    ...
    Trades MatchOrder(OrderModify order) {
        ...
        // FIX (lifetime): copy what we need OUT of the binding before CancelOrder
        //      erases the entry, otherwise existingOrder dangles when we read its type.
        auto existingType = existingOrder->GetOrderType();
